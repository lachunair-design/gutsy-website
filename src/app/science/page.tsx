...
export default function SciencePage() {
  ...
  return (
    <div className="bg-linen min-h-screen text-black selection:bg-red/10 overflow-x-hidden relative font-uto">
      ...
      {/* MOBILE NAVIGATOR */}
      <div className="lg:hidden sticky top-[80px] z-[80] bg-linen/95 backdrop-blur-xl border-b border-black/5 py-3 shadow-sm">
      ...
      {/* HERO SECTION */}
      <section className="bg-black text-linen pt-24 pb-16 md:pt-48 md:pb-32 relative overflow-hidden z-10">
        ...
        <Microscope className="absolute bottom-0 right-0 p-12 opacity-10 hidden md:block animate-pulse w-96 h-96 text-yellow stroke-[0.5]" />
      </section>
      ...
      <section className="py-12 md:py-20 border-b border-black/5 relative z-10">
        <div className="mx-auto max-w-7xl px-8 flex flex-col md:flex-row gap-12 items-center">
          <p className="text-xl md:text-2xl leading-relaxed text-black/80 italic max-w-3xl">
            Everything on this page is backed by third-party lab testing, supplier clinical data, and ingredient science. We wrote it so you don&apos;t have to wade through a 60-page COA to find out what&apos;s actually in your scoop. <span className="text-red font-black uppercase">You&apos;re welcome.</span>
          </p>
          <Zap className="hidden md:block text-red w-20 h-20 animate-bounce" />
        </div>
      </section>
          ...
          <div className="flex flex-col lg:flex-row gap-20">
            ...
            <aside className="hidden lg:block w-64 flex-shrink-0 relative">
              <div className="sticky top-40 h-fit border-l border-black/5 pl-6">
                <p className="text-[10px] uppercase font-black tracking-widest text-black/40 mb-6">The Evidence</p>
                <div className="flex flex-col gap-4">
                  {facts.map((f) => (
                    <button 
                      ...
                      onClick={() => scrollTo(f.id)} 
                      className={cn(
                        "block text-left text-xs uppercase tracking-wider transition-all hover:text-red transform font-bold",
                        activeSection === f.id ? "text-red translate-x-2" : "text-black/40"
                      )}
                    >
                      {f.num}. {f.id.replace('-', ' ')}
                    </button>
              ...
              {facts.map((fact) => (
                <div key={fact.num} id={fact.id} className="grid grid-cols-1 lg:grid-cols-10 gap-4 md:gap-8 items-start group scroll-mt-48">
                  <div className="flex items-center gap-4 lg:col-span-1">
                    <span className={cn("text-4xl md:text-6xl text-red transition-opacity font-black", activeSection === fact.id ? "opacity-100" : "opacity-10")}>
                      {fact.num}
                    </span>
                  ...
                  <div className="lg:col-span-9 max-w-2xl">
                    <h2 className="text-2xl md:text-5xl mb-6 leading-tight font-black uppercase tracking-tighter">{fact.title}</h2>
                    ...
                    {fact.hasVisual && (
                      <div className="my-8 aspect-square md:aspect-video flex flex-col items-center justify-center border border-dashed border-black/10 rounded-[32px] text-[10px] uppercase tracking-[0.2em] text-black/30 bg-white/20 backdrop-blur-sm italic">
                         {fact.hasVisual === 'hydrolysis' ? (
                           <>
                             <Beaker className="w-12 h-12 mb-4 opacity-20" />
                             [Visual: Molecule Breakdown — From Chains to Peptides]
                           </>
                         ) : (
                           <>
                             <Zap className="w-12 h-12 mb-4 opacity-20" />
                             [Visual: Enzyme Synergy — Actinidin vs Protein]
                           </>
                         )}
                      </div>
                    )}
                    ...
                    {fact.hasTable === 'metals' && (
                      <div className="mt-8 bg-white rounded-[32px] p-8 shadow-sm border border-black/5 relative overflow-hidden group/table">
                        <div className="flex items-center justify-between mb-8">
                          <p className="text-xs uppercase font-black tracking-widest">Laboratory Batch Analysis</p>
                          ...
                          {[
                            ...
                            <div key={i} className="flex flex-col gap-2 border-b border-black/5 pb-6 last:border-0">
                              ...
                              <div className="flex justify-between items-baseline">
                                <span className="text-3xl font-black">{row.c} <span className="text-xs opacity-30">mg/kg</span></span>
                                <span className="text-[10px] font-black text-red uppercase">{row.l}</span>
                              </div>
                            </div>
                          ))}
                        </div>
                    ...
                    {fact.hasTable === 'macros' && (
                      <div className="mt-8 grid grid-cols-2 gap-4 md:gap-8">
                        <div className="bg-black text-linen rounded-2xl p-6 border border-white/5"><p className="opacity-50 text-[10px] uppercase mb-1">Protein</p><p className="text-3xl md:text-4xl font-black">23g</p></div>
                        <div className="bg-black text-linen rounded-2xl p-6 border border-white/5"><p className="opacity-50 text-[10px] uppercase mb-1">Sugars</p><p className="text-3xl md:text-4xl font-black">&lt;1g</p></div>
                        <div className="bg-black text-linen rounded-2xl p-6 border border-white/5"><p className="opacity-50 text-[10px] uppercase mb-1">Vanilla</p><p className="text-3xl md:text-4xl text-yellow font-black">133 <span className="text-xs">kcal</span></p></div>
                        <div className="bg-black text-linen rounded-2xl p-6 border border-white/5"><p className="opacity-50 text-[10px] uppercase mb-1">Cacao</p><p className="text-3xl md:text-4xl text-yellow font-black">137 <span className="text-xs">kcal</span></p></div>
                      </div>
                    )}
                  </div>
                </div>
      ...
      <section className="py-20 bg-white relative z-10 border-y border-black/5">
        <div className="mx-auto max-w-4xl px-8 text-center">
          <Thermometer className="w-10 h-10 mx-auto mb-6 text-red" />
          <h2 className="text-4xl md:text-6xl mb-4 tracking-tighter font-black uppercase">The Bloat Scale</h2>
          <p className="text-lg text-black/60 mb-12 italic">Be honest. How do you feel 20 minutes after your current shake?</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              <button key={q.s} onClick={() => setQuizScore(q.s)} className={cn("p-8 border rounded-[32px] transition-all text-left flex items-center justify-between group", quizScore === q.s ? "bg-black text-white border-black scale-[0.98]" : "bg-linen border-black/5 hover:border-black/20")}>
                <div>
                  <span className="text-2xl block mb-2 font-black uppercase">{q.l}</span>
                  <p className="opacity-60 text-xs">{q.d}</p>
                </div>
                <ChevronRight className={cn("w-5 h-5 transition-transform", quizScore === q.s ? "rotate-90 text-red" : "opacity-20")} />
              </button>
            ))}
          </div>

          {quizScore && (
            <div className="mt-8 p-10 bg-red text-linen rounded-[40px] text-left animate-in slide-in-from-bottom-4 duration-500">
              <p className="text-2xl md:text-3xl mb-4 font-black uppercase">
      ...
      <section className="py-20 bg-black text-white/30 relative z-10">
        <div className="mx-auto max-w-7xl px-8">
            <div className="flex items-center gap-3 mb-12 text-yellow">
              <Beaker className="w-5 h-5" />
              <p className="text-xs uppercase tracking-[0.4em] font-black">Scientific Verification Panel</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-8 gap-x-20">
                {[
                  "Actazin® clinical data: Anagenix Limited, New Zealand",
                  "Protein quality and hydrolysis data: ZymeBase Inc.",
                  "Third-party laboratory testing: ISO 17025-accredited laboratory, 2025",
                  ...
                  <div key={i} className="flex items-start gap-4 py-4 border-t border-white/10">
                    ...
                    <span className="text-[10px] uppercase tracking-widest leading-relaxed font-bold">{s}</span>
                  </div>
                ))}
            </div>
        </div>
