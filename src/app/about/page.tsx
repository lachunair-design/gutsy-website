...
export default function AboutPage() {
  ...
  // Illustration Parallax: Enlarged and slowed down for depth
  const yMove = useTransform(scrollYProgress, [0, 1], [0, 400]);
  const scale = useTransform(scrollYProgress, [0, 0.5], [1.5, 1.2]);
  ...
  return (
    <div className="bg-[#f3eee4] min-h-screen p-4 md:p-6 lg:p-8 pt-8 space-y-8 selection:bg-[#ffb300] selection:text-black">
      ...
      <div ref={containerRef} className={`bg-[#f20028] rounded-[40px] md:rounded-[60px] lg:rounded-[80px] overflow-hidden relative ${utoMedium.className}`}>
        ...
        {/* BACKGROUND ILLUSTRATION - ENLARGED & PUSHED BACK */}
        <motion.div 
        ...
        <div className="mx-auto max-w-6xl px-6 lg:px-8 pt-48 pb-40 relative z-10">
          
          {/* HERO SECTION - REFINED STAGGERING */}
          <div className="relative mb-80 flex flex-col items-center">
            <h2 className={`text-[#f3eee4] text-6xl md:text-9xl lowercase tracking-tight mb-[-3rem] mr-[30%] ${runWild.className} rotate-[-5deg]`}>
              the big fat
            </h2>
            <h1 className={`text-black text-[120px] md:text-[280px] leading-[0.7] uppercase tracking-tighter text-center ${utoBlack.className}`}>
              GUTSY
            </h1>
            <h2 className={`text-[#f3eee4] text-6xl md:text-9xl lowercase tracking-tight mt-[-4rem] ml-[40%] ${runWild.className} rotate-[4deg]`}>
              backstory
            </h2>
          </div>

          {/* STORY GRID - BROUGHT UP TO FLOAT OVER BACKGROUND */}
      ...
      <section className="bg-black rounded-[40px] md:rounded-[60px] lg:rounded-[80px] overflow-hidden relative py-32 border-4 border-[#f3eee4] shadow-[15px_15px_0px_0px_#ffb300]">
        <div className="mx-auto max-w-3xl px-6 text-center relative z-10 space-y-10">
          ...
          <p className={`text-6xl lowercase text-[#ffb300] leading-none ${runWild.className}`}>get 10% off your first order</p>
